...
/**
 *  初始化静态结构 static NXHashTablePrototype protoPrototype
 */
...
/**
 *  初始化静态结构体指针 static NXHashTable *prototypes
 */
...
NXHashTable *NXCreateHashTableFromZone (NXHashTablePrototype prototype, unsigned capacity, const void *info, void *z)
{
    ...
    // table = ((NXHashTable *)malloc(sizeof (NXHashTable)));
    table = ALLOCTABLE(z);
...
bool NXCompareHashTables (NXHashTable *table1, NXHashTable *table2)
{
    ...
    if (NXCountHashTable (table1) != NXCountHashTable (table2)) return false;
    else {
        void		*data;
        NXHashState	state = NXInitHashState (table1);
        while (NXNextHashState (table1, &state, &data)) {
            if (! NXHashMember (table2, data)) return false;
        }
        return true;
    }
}
